  ...
  <body>
    ...
    <script type="text/javascript">
      	$(document).ready(function() {
			...
			{
				  $.ajax({
				   url:"<?php echo base_url(); ?>home/pagination_recent_products/"+page,
				   method:"GET",
				   dataType:"json",
				   beforeSend:function()
	               {
	        ...
	        {
	            $.ajax({
	             url:"<?php echo base_url(); ?>home/fetch_search_product_pagination_shop",
	             method:"POST",
	             data:{query:query},
	              beforeSend:function()
	              {
	        ...
	        $(document).on("click",".add_cart", function(){
	 	    ...
	 	    $(document).on('click', '.add_favory', function(event) {
	 	    	...
	 	    	if (product_id !='') {
	 	    		...
	 	    		$.ajax({
					    url:"<?php echo base_url(); ?>home/add_favories",
					    method:"POST",
					    data:{
					    	product_id:product_id
					    },
	 	    ...
	 	    $(document).on('click', '.modalView', function(event) {
	 	    	...
	 	    	$.ajax({  
                    url:"<?php echo base_url(); ?>home/fetch_single_article_modal",  
                    method:"POST",  
	 	    ...
	 	    function show_photo_galery(product_id){
	 	    	if (product_id !='') {
	 	    		$.ajax({  
	                    url:"<?php echo base_url(); ?>home/fetch_single_galery_photo",  
	                    method:"POST",  
	                    data:{product_id:product_id},  
	                    success:function(data)  
	                    {  
	 	    ...
	 	    function show_photo_product(){
 	    	    $.ajax({  
                    url:"<?php echo base_url(); ?>home/fetch_single_rand_product",  
                    method:"POST",  
                    data:{}, 
                    beforeSend:function()
		            {
	 	    ...
	 	    $(document).on('click','.brand_product_name', function(){
				...
				// alert("band "+brand);
				$.ajax({
					url:"<?php echo base_url(); ?>home/view_all_product_by_cat_product_id",
					method: "POST",
					data:{
						brand: brand
					},
			...
			$(document).on('click','.brand_category_id', function(){
				...
				// alert("band "+brand);
				$.ajax({
					url:"<?php echo base_url(); ?>home/view_all_product_by_cat_category_id",
					method: "POST",
					data:{
						brand: brand
					},
			...
			$(document).on('click','.brand_product_price', function(){
				...
				// alert("band "+brand);
				$.ajax({
					url:"<?php echo base_url(); ?>home/view_all_product_by_cat_product_price",
					method: "POST",
					data:{
						brand: brand
					},
			...
			$(document).on('click', '.remove_inventory', function(){
			  var idc = $(this).attr("id");
			    ...
			    if(confirm("Êtes-vous sûr? Vous ne pourrez pas récupérer ces données provinciales!"))
	            {
	           		...
	           		$.ajax({
	                  url:"<?php echo base_url(); ?>home/remove_product_to_cart",
	                  method:"POST",
	                  data:{idc:idc},
	                  success:function(data)
	                  {
			...
			$(document).on('click', '#clear_cart', function(){
				...
				if(confirm("Êtes-vous sûr de tous supprimer? Vous ne pourrez pas récupérer ces données provinciales!"))
	            {
	           		...
	           		$.ajax({
	                  url:"<?php echo base_url(); ?>home/clear_my_card",
	                  method:"POST",
	                  success:function(data)
	                  {
			...
			$(document).on('click','.panier_show', function(){
		 	...
		 	function show_card(){
				$('#country_table').load("<?php echo base_url(); ?>home/load_cart");
		 	}

		 	function publication_rand(){
		 		$.ajax({  
                    url:"<?php echo base_url(); ?>home/rand_publication_product",  
                    method:"POST",  
                    data:{}, 
                    beforeSend:function()
		            {
		 	...
		 	$(document).on('keyup', '.qte_modifiy', function(event) {
		 		...
		 		/* Act on the event */
		 		if (event.which == 13) {
		 			var url = "<?php echo(base_url()) ?>home/edit_my_panier";
